...
inline bool AdaptivePool2DInferShape(const nnvm::NodeAttrs& attrs,
  ...
  auto output_size = ShapeToArray(param.output_size);
  ...
  TShape oshape = dshape;
//  CHECK(output_size[0] <= dshape[hidx])
//      << "output height (" << output_size[0] << ") exceeds input ("
//      << dshape[hidx] << ")";
//  CHECK(output_size[1] <= dshape[widx])
//      << "output width (" << output_size[1] << ") exceeds input ("
//      << dshape[widx] << ")";

  oshape[hidx] = param.output_size[0];
  oshape[widx] = param.output_size[1];
...
.set_attr<FTVMCompute>("FTVMCompute", [](const NodeAttrs& attrs,
  ...
  return Array<Tensor>{
    topi::nn::adaptive_pool(inputs[0], output_size,
