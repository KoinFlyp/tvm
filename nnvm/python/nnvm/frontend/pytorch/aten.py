r'''This file contains one class per PyTorch ATen operator. For the full
...
class Conv2D(ATenOp):
    ...
    def __init__(self, node, graph):
        ...
        if self.get_input(6).get_output(0):
            ...
            topi_name = 'conv2d'
        inputs = [self.get_input(i).get_output(0) for i in [0, 1, 2]]
        if inputs[2] is None:
            bias = np.zeros([self.inputs[1].shape[0]]).astype('float32')
            self.graph.add_param(self.get_input_name(2), bias)
            inputs[2] = self.graph[self.get_input_name(2)].get_output(0)
        weight_shape = self.get_input(1).shape
        attrs = {
...
class BatchNorm(ATenOp):
    ...
    def __init__(self, node, graph):
        ...
        inputs = [self.get_input(i).get_output(0) for i in range(5)]
        attrs = {
...
class LogSoftmax(ATenOp):
    ...
    def __init__(self, node, graph):
        ...
        inputs = [self.get_input(0).get_output(0)]
        self.set_output(0, self.get_output_name(0),
                        make_symbol('log_softmax', *inputs))
...
class Dense(ATenOp):
    ...
    def __init__(self, node, graph):
        ...
        inputs = [self.get_input(i).get_output(0) for i in [1, 2, 0]]
        units = self.get_input(2).shape[1]
        attrs = {
            'units': units,
        }
        ...
        inputs[0] *= alpha
        inputs[1] = make_symbol('transpose', beta * inputs[1], axes=[1, 0])
        self.set_output(0, self.get_output_name(0),
