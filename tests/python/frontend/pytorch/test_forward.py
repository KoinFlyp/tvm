...
def verify_model(model_name):
    ...
    baseline_input = baseline_input.cpu()
    trace = torch.jit.trace(baseline_model, baseline_input).float().eval().cpu()
    ...
    with nnvm.compiler.build_config(opt_level=3, ext_accel=EXT_ACCEL):
...
def verify_batchnorm2():
    verify_model('BatchNorm2')

def verify_batchnorm3():
    verify_model('BatchNorm3')
...
def verify_mobilenet_v1():
    verify_model('mobilenet_v1')

def verify_mobilenet_v2():
